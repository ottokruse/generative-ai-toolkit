...
###
# Start this script by running: python examples/sample_agent.py
###
...
from generative_ai_toolkit.tracer import HumanReadableTracer, InMemoryTracer, TeeTracer
...
agent = BedrockConverseAgent(
    ...
    tracer=TeeTracer().add_tracer(InMemoryTracer()).add_tracer(HumanReadableTracer()),
)
...
print(
    textwrap.dedent(
        ...
        # SAMPLE AGENT
        #
        # Please have some fun chatting with the sample agent.
        # The sample agent will log traces as it operates, so you can see what it is doing exactly.
        # Press enter to fire off your first message to the assistant.
        # The conversation will stop when you provide empty input.
        ###########
        """
    ).strip()
)
...
user_input = "Hi there assistant! What can you do for me?"

input(f"User: {user_input}").strip()

while user_input:
    for index, chunks in enumerate(agent.converse_stream(user_input)):
        if index == 0:
